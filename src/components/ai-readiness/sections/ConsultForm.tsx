...
const MSG_MAX = 500;
const FORM_ID = "ai-readiness-consult";
...
const FIELDS = {
  ...
  aiMaturity: { rule: "aiMaturity" as const, required: true },
  consent: { rule: "consent" as const, required: true },
};

type AiMaturity = "" | "none" | "pilot" | "production";
...
const MATURITY_OPTIONS: Array<{
  ...
  {
    value: "none",
    label: "Zvažujeme možnosti",
    hint: "Mapujeme, kam AI nasadit nejdřív — chceme to udělat dobře",
  },
  {
    value: "pilot",
    label: "Pilot ve vybraných týmech",
    hint: "Testujeme AI nástroje v 1–2 týmech, vyhodnocujeme",
  },
  {
    value: "production",
    label: "Produkce napříč firmou",
    hint: "AI nástroje běží oficiálně víc než půl roku",
  },
];
...
export default function ConsultForm({ rep }: { rep?: Rep }) {
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <p
          className="mt-4 max-w-[46ch] mx-auto leading-[1.55]"
          style={{ fontSize: 15, color: "rgba(28,18,55,0.7)" }}
        >
          {rep ? `${rep.name.split(" ")[0]} vám pošle` : "Náš konzultant vám pošle"}
          {" 2–3 termíny pro 15minutový hovor o AI readiness na vámi uvedený e-mail."}
        </p>
        {rep && (
          <div
  ...
  const aiMaturityError = validation.errors.aiMaturity;
  ...
  return (
    ...
    >
      ...
      {/* AI maturity radio group */}
      <div data-field="aiMaturity" className="mt-5">
        <FieldLabel>
          Kde jste dnes s AI?
        ...
        <div
          ...
          role="radiogroup"
          aria-required="true"
          aria-invalid={!!aiMaturityError}
          aria-describedby={aiMaturityError ? "aimaturity-error" : undefined}
        >
          {MATURITY_OPTIONS.map((opt) => {
            const selected = aiMaturity === opt.value;
            return (
              <label
                key={opt.value}
                className="cursor-pointer flex flex-col gap-1 select-none"
                style={{
                  padding: "12px 14px",
                  borderRadius: 8,
                  background: selected
                    ...
                    : "var(--color-paper)",
                  border: selected
                    ? "1.5px solid var(--color-purple-deep)"
              ...
              >
                <input
                  ...
                  checked={selected}
                  onChange={() => {
                    setAiMaturity(opt.value);
                    validation.onChange("aiMaturity");
                  }}
                  onBlur={() => validation.onBlur("aiMaturity")}
                  className="sr-only"
                />
                <span
                  style={{
                    fontSize: 14,
                    fontWeight: 600,
                    color: selected ? "var(--color-purple-deep)" : "var(--color-ink)",
                    lineHeight: 1.25,
                  }}
                >
                  {opt.label}
      ...
      <div className="mt-4">
        ...
        <textarea
          ...
          rows={3}
          placeholder="V jaké fázi AI rolloutu jste, co řešíte, kdo je sponzor…"
