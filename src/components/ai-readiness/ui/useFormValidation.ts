...
export const rules = {
  ...
  aiMaturity: (v: string): FieldStatus => {
    if (!v || !["none", "pilot", "production"].includes(v)) {
      return { status: "error", msg: "Vyberte prosím, kde jste dnes s AI." };
    }
    return { status: "ok" };
  },

  consent: (v: boolean): FieldStatus => {
    if (!v) return { status: "error", msg: "Bez souhlasu vás nemůžeme kontaktovat." };
    return { status: "ok" };
  },
} as const;
